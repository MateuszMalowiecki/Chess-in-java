...
public class Goniec extends Figura {
    public Goniec(Druzyna d) {super(d);}
    public Goniec(int posx, int posy, Druzyna d) {
        super(posx, posy, d);
    }
    @Override
    public List<Ruch> mozliweRuchy() {
        ...
        if (this.posy < Plansza.ROZMIAR-1 && this.posx > 0) {
            if (p.getFigura(this.posy + 1, this.posx - 1) == null || p.getFigura(this.posy + 1, this.posx - 1).druzyna != this.druzyna) {
                b=p.getFigura(this.posy+1, this.posx-1) != null;
                addOnList(l, new Ruch(this.posy + 1, this.posx - 1, b));
    ...
    public boolean przemieszczenie() throws InvalidAlgorithmParameterException {
        ...
        if (l.size() == 0) {
            ...
            System.out.println("Goniec zostal zbity");
        }
        return false;
    }
    @Override
    public String toString() {
        if (this.druzyna == Druzyna.CZARNE) return "rc";
        return "rb";
    }
}